...
class TestBoard(unittest.TestCase):
    ...
    def test_level_elements_in_board(self):
        ...
        board_positions = []
        self.__board.add_level_statics(first_level.elements)
        for row in self.__board.squares:
            for position in row:
                board_positions.append(position)
                self.assertIsInstance(position.element, Entity)
        self.assertEqual(len(first_level.elements), len(board_positions))
        
        first_level_board_positions = copy.deepcopy(self.__board.squares)
        ...
        if len(LEVELS) > int(FIRST_LEVEL):
            self.__board.add_level_statics(next_level.elements)
            self.assertNotEqual(first_level_board_positions, self.__board.squares)
        else:
    ...
    def test_tank_in_board(self):
        ...
        with self.assertRaises(IndexError) as context:
            tank = Tank(column=TEST_OUT_OF_LIMITS_COLUMN, row=TEST_OUT_OF_LIMITS_ROW, direction=PLAYER_SPAWN_DIRECTION, speed=PLAYER_SPAWN_SPEED)
            self.__board.squares[TEST_OUT_OF_LIMITS_ROW][TEST_OUT_OF_LIMITS_COLUMN].tank = tank
        ...
        with self.assertRaises(IndexError) as context:
            tank_in_position = self.__board.squares[TEST_OUT_OF_LIMITS_ROW][TEST_OUT_OF_LIMITS_COLUMN].tank
            self.assertIsNone(tank_in_position)

        tank = Tank(column=TEST_VALID_COLUMN, row=TEST_VALID_ROW, direction=PLAYER_SPAWN_DIRECTION, speed=PLAYER_SPAWN_SPEED)
        self.__board.add_tank(tank)
        self.__board.squares[TEST_VALID_ROW][TEST_VALID_COLUMN].tank = tank
        ...
        tank_in_position_2 = self.__board.squares[TEST_VALID_ROW][TEST_VALID_COLUMN].tank

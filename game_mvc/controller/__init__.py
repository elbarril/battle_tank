...
class GameController:
    ...
    def __play_level(self):
        ...
        for row in map:
            for object in row:
                if not object or object in self.__object_views:
                    continue
                if isinstance(object, CompoundMapObject):
                    for obj in object:
                        x, y = obj.position
                        width, height = obj.size
                        object_view = map_view.create_object(
                            x, y, width, height, obj.image, obj.color, obj.layer)
                        self.__object_views.setdefault(obj, object_view)
                else:
                    x, y = object.position
                    width, height = object.size
                    object_view = map_view.create_object(
                        x, y, width, height, object.image, object.color, object.layer)
    ...
    def __player_shoots(self, player: AbstractPlayer):
        ...
        if not bullet_positions in map:
            return
        other_objects = [object for object in map[bullet_positions]
                         if object and not object is bullet and isinstance(object, SolidMapObject)]
        ...
        if other_objects:
            ...
            bullet_view = self.view.map.create_object(
                x, y, width, height, bullet.image, bullet.color, bullet.layer)
    ...
    def __player_moves(self, player: AbstractPlayer, direction: MapObjectDirection):
        ...
        if next_positions in map:
            other_objects = [object for object in map[next_positions]
                             if object and not object is tank and isinstance(object, SolidMapObject)]
            if other_objects:
                return
